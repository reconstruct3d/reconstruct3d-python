...
def MatchFeatures(self, idx_i, idx_j, matches):    
#ifdef __SFM__DEBUG__
     img_1 = imgs[idx_i]
     img_2 = imgs[idx_j]
#endif
     vector<KeyPoint>imgpts1 = imgpts[idx_i]
     vector<KeyPoint>imgpts2 = imgpts[idx_j]
     descriptors_1 = descriptors[idx_i]
     descriptors_2 = descriptors[idx_j]
...
#    cutoff = 4.0*min_dist
    ...
    { 
        ...
        if (*matches)[i].trainIdx <= 0:            (*matches)[i].trainIdx = (*matches)[i].imgIdx

        
        if( existing_trainIdx.find((*matches)[i].trainIdx) == existing_trainIdx.end() and 
           (*matches)[i].trainIdx >= 0 and (*matches)[i].trainIdx < (int)(keypoints_2.size()) '''and
           (*matches)[i].distance > 0.0 and (*matches)[i].distance < cutoff''' ) 
	...
	assert(imgpts1_good.size() == imgpts2_good.size() and imgpts1_good.size() == good_matches_.size())
