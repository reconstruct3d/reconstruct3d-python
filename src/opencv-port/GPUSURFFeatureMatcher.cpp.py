...
GPUSURFFeatureMatcher.GPUSURFFeatureMatcher(vector<cv.Mat>& imgs_, 
	...
	# The helper function printShortCudaDeviceInfo() moved between OpenCV v2.3 and v2.4, might not compile.
	#printShortCudaDeviceInfo(cv.gpu.getDevice())

	extractor = gpu.SURF_GPU()
	...
	CV_PROFILE("extract",
	for(int img_i=0;img_i<imgs_.size();img_i++)		GpuMat _m; _m.upload(imgs_[img_i])
...
def MatchFeatures(self, idx_i, idx_j, matches):	
	...
	if matches.size() == 0:		cout << "match " << descriptors_1.rows << " vs. " << descriptors_2.rows << " ..."
		...
		if use_ratio_test:			vector<vector<DMatch> > knn_matches
			...
			#ratio test
			for(int i=0;i<knn_matches.size();i++)				if knn_matches[i][0].distance / knn_matches[i][1].distance < 0.7:					(*matches).push_back(knn_matches[i][0])
