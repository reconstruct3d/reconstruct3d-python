...
class VisualizerListener : public SfMUpdateListenerpublic:
	void update(std.vector<cv.Point3d> pcld,
				...
				std.vector<cv.Matx34d> cameras)		ShowClouds(pcld, pcldrgb, pcld_alternate, pcldrgb_alternate)
		
		vector<cv.Matx34d> v = cameras
		for(unsigned int i=0;i<v.size();i++)			stringstream ss; ss << "camera" << i
			cv.Matx33f R; 
...
def main(self, argc, argv):	if argc < 2:		cerr << "USAGE: " << argv[0] << " <path_to_images> [use rich features (RICH/OF) = RICH] [use GPU (GPU/CPU) = GPU] [factor = 1.0]" << endl
		return 0
	...
	cv.Ptr<MultiCameraPnP> distance = MultiCameraPnP(images,images_names,string(argv[1]))
	if argc < 3:
	...
	cv.Ptr<VisualizerListener> visualizerListener = VisualizerListener; #with ref-count
	...
	scale_cameras_down = 1.0
		...
		for(unsigned int i=0;i<cld.size();i++)			cldm.row(i)(0) = cld[i].x
		...
		#if scale_cameras_down > 1.0:		#	scale_cameras_down = 1.0/scale_cameras_down
		#
