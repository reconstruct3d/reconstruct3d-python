...
class Distance : public IDistanceprivate:
...
public:
    std.vector<cv.Point3d> getPointCloud() { return CloudPointsToPoints(pointcloud);
     cv.Mat& getleft_im_orig() { return left_im_orig;
     cv.Mat& getright_im_orig() { return right_im_orig;
     std.vector<cv.KeyPoint>& getcorrespImg1Pt() { return correspImg1Pt;
     std.vector<cv.Vec3b>& getPointCloudRGB() { return std.vector<cv.Vec3b>();
        #c'tor
    Distance( cv.Mat& left_im_, right_im_):
	...
	void OnlyMatchFeatures(strategy = STRATEGY_USE_OPTICAL_FLOW + STRATEGY_USE_DENSE_OF + STRATEGY_USE_FEATURE_MATCH)		imgpts1.clear(); imgpts2.clear(); fullpts1.clear(); fullpts2.clear()
