...
MultiCameraDistance.MultiCameraDistance(
	 ...
	 std.vector<std.string>& imgs_names_, 
	 std.string& imgs_path_):
...
{		
	...
	for (unsigned int i=0; i<imgs_.size(); i++)		imgs_orig.push_back(cv.Mat_<cv.Vec3b>())
		if not imgs_[i].empty():			if imgs_[i].type() == CV_8UC1:				cvtColor(imgs_[i], imgs_orig[i], CV_GRAY2BGR)
			} elif imgs_[i].type() == CV_32FC3 or imgs_[i].type() == CV_64FC3:				imgs_[i].convertTo(imgs_orig[i],CV_8UC3,255.0)
			} else:
	...
	cv.FileStorage fs
	if fs.open(imgs_path_+ "\\out_camera_data.yml",cv.FileStorage.READ):		fs["camera_matrix"]>>cam_matrix
		fs["distortion_coefficients"]>>distortion_coeff
	} else:
		#no calibration matrix file - mockup calibration
		imgs_size = imgs_[0].size()
...
void MultiCameraDistance.OnlyMatchFeatures(int strategy) 
	...
	if use_rich_features:		if use_gpu:			feature_matcher = GPUSURFFeatureMatcher(imgs,imgpts)
		} else:
			feature_matcher = RichFeatureMatcher(imgs,imgpts)

	} else:
		feature_matcher = OFFeatureMatcher(use_gpu,imgs,imgpts)
	}	

	if strategy & STRATEGY_USE_OPTICAL_FLOW:
		use_rich_features = False

	loop1_top = imgs.size() - 1, loop2_top = imgs.size()
	frame_num_i = 0
	##pragma omp parallel for schedule(dynamic)
	
	#if use_rich_features:	#	for (frame_num_i = 0; frame_num_i < loop1_top; frame_num_i++)	#		for (frame_num_j = frame_num_i + 1; frame_num_j < loop2_top; frame_num_j++)
...
#pragma omp parallel for
		for (frame_num_i = 0; frame_num_i < loop1_top; frame_num_i++)			for (frame_num_j = frame_num_i + 1; frame_num_j < loop2_top; frame_num_j++)
				std.cout << "------------ Match " << imgs_names[frame_num_i] << ","<<imgs_names[frame_num_j]<<" ------------\n"
				std.vector<cv.DMatch> matches_tmp
				feature_matcher.MatchFeatures(frame_num_i,frame_num_j,&matches_tmp)
				matches_matrix[std.make_pair(frame_num_i,frame_num_j)] = matches_tmp
...
void MultiCameraDistance.GetRGBForPointCloud(
	 std.vector<struct CloudPoint>& _pcloud,
	std.vector<cv.Vec3b>& RGBforCloud
	) 
	RGBforCloud.resize(_pcloud.size())
	for (unsigned int i=0; i<_pcloud.size(); i++)		unsigned good_view = 0
		std.vector<cv.Vec3b> point_colors
		for(; good_view < imgs_orig.size(); good_view++)			if _pcloud[i].imgpt_for_img[good_view] != -1:				pt_idx = _pcloud[i].imgpt_for_img[good_view]
				if pt_idx >= imgpts[good_view].size():					std.cerr << "BUG: point id:" << pt_idx << " should not exist for img #" << good_view << " which has only " << imgpts[good_view].size() << std.endl
					continue

				_pt = imgpts[good_view][pt_idx].pt
				assert(good_view < imgs_orig.size() and _pt.x < imgs_orig[good_view].cols and _pt.y < imgs_orig[good_view].rows)
...
#		cv.waitKey(0)
		res_color = cv.mean(point_colors)
