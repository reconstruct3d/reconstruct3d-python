...
def LinearLSTriangulation(self, u,		#homogenous image point (u,v,1):
	...
	#build matrix A for homogenous equation Ax = 0
	#X = (x,y,z,1), Linear-LS method
	#which turns it into AX = B system, A is 4x3, is 3x1 and B is 4x1
	#	cout << "u " << u <<", u1 " << u1 << endl
	#	Matx<double,6, A; #self is for the AX=0 case, with linear dependence..
	#	A(0) = u.x*P(2)-P(0)
	#	A(1) = u.y*P(2)-P(1)
	#	A(2) = u.x*P(1)-u.y*P(0)
	#	A(3) = u1.x*P1(2)-P1(0)
...
def IterativeLinearLSTriangulation(self, u,	#homogenous image point (u,v,1):
	...
	for (int i=0; i<10; i++) { #Hartley suggests 10 iterations at most		
		...
		#recalculate weights
		p2x = Mat_<double>(Mat_<double>(P).row(2)*X)(0)
		p2x1 = Mat_<double>(Mat_<double>(P1).row(2)*X)(0)
		
		#breaking point
		if(fabsf(wi - p2x) <= EPSILON and fabsf(wi1 - p2x1) <= EPSILON) break
...
#Triagulate points
double TriangulatePoints( vector<KeyPoint>& pt_set1, 
...
#	pointcloud.clear()
	...
	#triangulate
	pt_set1_pt_2r = pt_set1_pt.reshape(1, 2)
	pt_set2_pt_2r = pt_set2_pt.reshape(1, 2)
	...
	for (unsigned int i=0; i<pts_size; i++)		CloudPoint cp; 
		cp.pt = pt_3d[i]
...
#pragma omp parallel for num_threads(1)
	for (int i=0; i<pts_size; i++)		kp = pt_set1[i].pt; 
...
#pragma omp critical
	...
	mse = mean(reproj_error)
	t = ((double)getTickCount() - t)/getTickFrequency()
	cout << "Done. ("<<pointcloud.size()<<"points, " << t <<"s, err = " << mse[0] << ")"<< endl
	...
	#show "range image"
		...
		for (unsigned int i=0; i<pointcloud.size(); i++)			_d = MAX(MIN((pointcloud[i].z-minVal)/(maxVal-minVal),1.0),0.0)
