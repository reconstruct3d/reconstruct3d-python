...
namespace
	...
	showErrorStatistics(double  f0,
		...
		for (k = 0; k < K; ++k)
			i = correspondingView[k]
			j = correspondingPoint[k]
			p = cams[i].projectPoint(distortion, Xs[j])
			
			reprojectionError = norm_L2(f0 * (p - measurements[k]))
...
def Count2DMeasurements(self, pointcloud):	K = 0
	for (unsigned int i=0; i<pointcloud.size(); i++)		for (unsigned int ii=0; ii<pointcloud[i].imgpt_for_img.size(); ii++)			if pointcloud[i].imgpt_for_img[ii] >= 0:				K ++
...
void BundleAdjuster.adjustBundle(vector<CloudPoint>& pointcloud, 
	...
	N = Pmats.size(), M = pointcloud.size(), K = Count2DMeasurements(pointcloud)
	...
	f0 = KMat[0][0]
	cout << "intrinsic bundle = "; displayMatrix(KMat)
	Knorm = KMat
	...
	for (unsigned k = 0; k < pointcloud.size(); ++k)
		for (unsigned int i=0; i<pointcloud[k].imgpt_for_img.size(); i++)			if pointcloud[k].imgpt_for_img[i] >= 0:				view = i, point = k
				Vector3d p, np
				
				cvp = imgpts[i][pointcloud[k].imgpt_for_img[i]].pt
				p[0] = cvp.x
				p[1] = cvp.y
				p[2] = 1.0
				
				if (camIdBwdMap.find(view) != camIdBwdMap.end() and
...
#	V3D.optimizerVerbosenessLevel = 1
	inlierThreshold = 2.0 / fabs(f0)
	
	K0 = cams[0].getIntrinsic()
	cout << "K0 = "; displayMatrix(K0)

	good_adjustment = False
...
#		StdMetricBundleOptimizer opt(inlierThreshold,cams,Xs,measurements,correspondingView,correspondingPoint)
	...
	cout << "K = "; displayMatrix(K0)
	
	for (i = 0; i < N; ++i) cams[i].setIntrinsic(K0)
	...
	if(good_adjustment) { #good adjustment?
		...
		#for (unsigned j = 0; j < Xs.size(); ++j) addVectorsIP(Xs[j], mean)
		...
		#distThr = norms[int(norms.size() * 0.9f)]
