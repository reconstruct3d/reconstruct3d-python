...
void OFFeatureMatcher.MatchFeatures(int idx_i, idx_j, matches) {	
	...
	Mat prevgray,gray
	if imgs[idx_i].channels() == 3:		cvtColor(imgs[idx_i],prevgray,CV_RGB2GRAY)
	...
	if use_gpu:		gpu.GpuMat gpu_prevImg,gpu_nextImg,gpu_prevPts,gpu_nextPts,gpu_status,gpu_error
	...
	for (unsigned int i=0; i<vstatus.size(); i++)		if vstatus[i] and verror[i] < 12.0:			to_find_back_idx.push_back(i)
	...
	to_find_flat = Mat(to_find).reshape(1,to_find.size())
	
	vector<Point2f> j_pts_to_find
	KeyPointsToPoints(imgpts[idx_j],j_pts_to_find)
	j_pts_flat = Mat(j_pts_to_find).reshape(1,j_pts_to_find.size())
	...
	CV_PROFILE("Prune",
	for(int i=0;i<knn_matches.size();i++)		DMatch _m
		if knn_matches[i].size()==1:			_m = knn_matches[i][0]
		} elif knn_matches[i].size()>1:			if knn_matches[i][0].distance / knn_matches[i][1].distance < 0.7:				_m = knn_matches[i][0]
			} else:
	...
	cout << "pruned " << matches.size() << " / " << knn_matches.size() << " matches" << endl
		...
		for(int i=0;i<matches.size();i++)			#if i%2 != 0:#				continue
#
			i_pt = imgpts[idx_i][(*matches)[i].queryIdx].pt
			j_pt = imgpts[idx_j][(*matches)[i].trainIdx].pt
...
#		putText(img_matches,ss.str(),Point(10,20),CV_FONT_HERSHEY_PLAIN,1.0,Scalar(255),2)
		...
		c = waitKey(0)
		if c=='s':			imwrite(ss.str(), img_matches)
