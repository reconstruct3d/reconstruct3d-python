...
public:
	...
	std.vector<cv.Point3d> getPointCloud() { return CloudPointsToPoints(pcloud);
	 cv.Mat& get_im_orig(int frame_num) { return imgs_orig[frame_num];
	 std.vector<cv.KeyPoint>& getcorrespImg1Pt() { return correspImg1Pt;
	 std.vector<cv.Vec3b>& getPointCloudRGB() { if(pointCloudRGB.size()==0) { GetRGBForPointCloud(pcloud,pointCloudRGB); } return pointCloudRGB;
	std.vector<cv.Matx34d> getCameras() { 
		std.vector<cv.Matx34d> v; 
		for(std.map<int , it = Pmats.begin(); it != Pmats.end(); ++it )			v.push_back( it.second )

		return v


	void GetRGBForPointCloud(
		 std.vector<struct CloudPoint>& pcloud,
	...
	virtual void OnlyMatchFeatures(strategy = STRATEGY_USE_FEATURE_MATCH);	
