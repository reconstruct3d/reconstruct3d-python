...
int MultiCameraPnP.FindHomographyInliers2Views(int vi, vj) 
 ...
 * Get an initial 3D point cloud from 2 views only
 '''
def GetBaseLineTriangulation(self):	std.cout << "=========================== Baseline triangulation ===========================\n"
	...
	#TODO: parallelize!
	for(std.map<std.pair<int,int> ,std.vector<cv.DMatch> >i = matches_matrix.begin(); i != matches_matrix.end(); ++i)		if (*i).second.size() < 100:
	...
	for(list<pair<int,pair<int,int> > >highest_pair = matches_sizes.begin(); 
		highest_pair != matches_sizes.end() and not goodF; 
		++highest_pair) 
		...
		if goodF:			vector<CloudPoint> new_triangulated
			vector<int> add_to_cloud

			Pmats[m_first_view] = P
			Pmats[m_second_view] = P1

			good_triangulation = TriangulatePointsBetweenViews(m_second_view,m_first_view,new_triangulated,add_to_cloud)
			if not good_triangulation or cv.countNonZero(add_to_cloud) < 10:				std.cout << "triangulation failed" << std.endl
				goodF = False
				...
				for (unsigned int j=0; j<add_to_cloud.size(); j++)					if add_to_cloud[j] == 1:
	...
	if not goodF:		cerr << "Cannot find a good pair of images to obtain a baseline triangulation" << endl
		exit(0)
...
#										 correspImg1Pt)
#		
#		for (unsigned int i=0; i<pcloud.size(); i++)#			pcloud[i].imgpt_for_img = std.vector<int>(imgs.size(),-1)
...
void MultiCameraPnP.Find2D3DCorrespondences(int working_view, 
	...
	for (set<int>done_view = good_views.begin(); done_view != good_views.end(); ++done_view) 
		old_view = *done_view
		...
		for (unsigned int match_from_old_view=0; match_from_old_view < matches_from_old_to_working.size(); match_from_old_view++)			# the index of the matching point in <old_view>
			idx_in_old_view = matches_from_old_to_working[match_from_old_view].queryIdx

			#scan the existing cloud (pcloud) to see if self point from <old_view> exists
			for (unsigned int pcldp=0; pcldp<pcloud.size(); pcldp++)				# see if corresponding point was found in self point
				if (idx_in_old_view == pcloud[pcldp].imgpt_for_img[old_view] and pcloud_status[pcldp] == 0) #prevent duplicates
					#3d point in cloud
...
bool MultiCameraPnP.FindPoseEstimation(
	...
	if(ppcloud.size() <= 7 or imgPoints.size() <= 7 or ppcloud.size() != imgPoints.size()) { 
	...
	if(inliers.size()==0) { #get inliers
		for(int i=0;i<projected3D.size();i++)			if norm(projected3D[i]-imgPoints[i]) < 10.0:
				inliers.push_back(i)
	...
	for(int ppt=0;ppt<imgPoints.size();ppt++)		cv.line(reprojected,imgPoints[ppt],projected3D[ppt],cv.Scalar(0,0,255),1)

	for (int ppt=0; ppt<inliers.size(); ppt++)		cv.line(reprojected,imgPoints[inliers[ppt]],projected3D[inliers[ppt]],cv.Scalar(0,0,255),1)

	for(int ppt=0;ppt<imgPoints.size();ppt++)		cv.circle(reprojected, imgPoints[ppt], 2, cv.Scalar(255,0,0), CV_FILLED)
		cv.circle(reprojected, projected3D[ppt], 2, cv.Scalar(0,255,0), CV_FILLED);			

	for (int ppt=0; ppt<inliers.size(); ppt++)		cv.circle(reprojected, imgPoints[inliers[ppt]], 2, cv.Scalar(255,255,0), CV_FILLED)
	...
	if inliers.size() < (double)(imgPoints.size())/5.0:		cerr << "not enough inliers to consider a good pose ("<<inliers.size()<<"/"<<imgPoints.size()<<")"<< endl
		return False


	if cv.norm(t) > 200.0:		# self is bad...
		cerr << "estimated camera movement is too big, self camera\r\n"
		return False


	cv.Rodrigues(rvec, R)
	if not CheckCoherentRotation(R):		cerr << "rotation is incoherent. we should try a different base view..." << endl
		return False
...
bool MultiCameraPnP.TriangulatePointsBetweenViews(
	...
	reproj_error = TriangulatePoints(pt_set1, pt_set2, K, Kinv, distortion_coeff, P, P1, new_triangulated, correspImg1Pt)
	std.cout << "triangulation reproj error " << reproj_error << std.endl

	vector<uchar> trig_status
	if not TestTriangulation(new_triangulated, P, trig_status) or not TestTriangulation(new_triangulated, P1, trig_status):		cerr << "Triangulation did not succeed" << endl
		return False

#	if reproj_error > 20.0:#		# somethign went awry, those triangulated points
...
#
	...
	reprj_err_cutoff = reprj_errors[4 * reprj_errors.size() / 5] * 2.4; #threshold from Snavely07 4.2
	
	vector<CloudPoint> new_triangulated_filtered
	std.vector<cv.DMatch> new_matches
	for(int i=0;i<new_triangulated.size();i++)		if trig_status[i] == 0:
			continue; #point was not in front of camera
		if new_triangulated[i].reprojection_error > 16.0:			continue; #reject point
		} 
		if(new_triangulated[i].reprojection_error < 4.0 or
	...
	matches = new_matches
	matches_matrix[std.make_pair(older_view,working_view)] = new_matches; #just to make sure, if unneccesary
	...
	#scan triangulated points, they were already triangulated before - strengthen cloud
	##pragma omp parallel for num_threads(1)
	for (j = 0; j<new_triangulated.size(); j++)		new_triangulated[j].imgpt_for_img = std.vector<int>(imgs.size(),-1)
		...
		for (unsigned view_ = 0; view_ < num_views; view_++)			if view_ != older_view:				#Look for points in <view_> that match to points in <working_view>
				std.vector<cv.DMatch> submatches = matches_matrix[std.make_pair(view_,working_view)]
				for (unsigned ii = 0; ii < submatches.size(); ii++)					if (submatches[ii].trainIdx == matches[j].trainIdx and
						not found_in_other_view) 
						#Point was already found in <view_> - strengthen it in the known cloud, it exists there

						#cout << "2d pt " << submatches[ii].queryIdx << " in img " << view_ << " matched 2d pt " << submatches[ii].trainIdx << " in img " << i << endl
						for (unsigned int pt3d=0; pt3d<pcloud.size(); pt3d++)							if (pcloud[pt3d].imgpt_for_img[view_] == submatches[ii].queryIdx) 
								#pcloud[pt3d] - a point that has 2d reference in <view_>

								#cout << "3d point "<<pt3d<<" in cloud, 2d pt " << submatches[ii].queryIdx << " in view " << view_ << endl
#pragma omp critical 
...
#pragma omp critical
			if found_in_other_view:				found_other_views_count++
			} else:
				add_to_cloud[j] = 1
...
def AdjustCurrentBundle(self):	cout << "======================== Bundle Adjustment ==========================\n"
	...
	cout << "use K " << endl << K << endl
	
	GetRGBForPointCloud(pcloud,pointCloudRGB)
}	

def PruneMatchesBasedOnF(self):	#prune the match between <_i> and all views using the Fundamental matrix to prune
##pragma omp parallel for
	for (int _i=0; _i < imgs.size() - 1; _i++)
		for (unsigned int _j=_i+1; _j < imgs.size(); _j++)			older_view = _i, working_view = _j
...
def RecoverDepthFromImages(self):	if(not features_matched) 
		OnlyMatchFeatures()
	...
	while (done_views.size() != imgs.size())
		...
		for (unsigned int _i=0; _i < imgs.size(); _i++)			if(done_views.find(_i) != done_views.end()) continue; #already done with self view

			vector<cv.Point3f> tmp3d; vector<cv.Point2f> tmp2d
			cout << imgs_names[_i] << ": "
			Find2D3DCorrespondences(_i,tmp3d,tmp2d)
			if tmp3d.size() > max_2d3d_count:				max_2d3d_count = tmp3d.size()
				max_2d3d_view = _i
				max_3d = tmp3d; max_2d = tmp2d


		i = max_2d3d_view; #highest 2d3d matching view

		std.cout << "-------------------------- " << imgs_names[i] << " --------------------------\n"
		done_views.insert(i); # don't repeat it for now

		pose_estimated = FindPoseEstimation(i,rvec,t,R,max_3d,max_2d)
		...
		for (set<int>done_view = good_views.begin(); done_view != good_views.end(); ++done_view) 
			...
			vector<int> add_to_cloud
			good_triangulation = TriangulatePointsBetweenViews(i,view,new_triangulated,add_to_cloud)
			if(not good_triangulation) continue

			std.cout << "before triangulation: " << pcloud.size()
			for (int j=0; j<add_to_cloud.size(); j++)				if add_to_cloud[j] == 1:
					pcloud.push_back(new_triangulated[j])
