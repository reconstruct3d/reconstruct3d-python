...
def DecomposeEssentialUsingHorn90(self, _E[9], _R1[9], _R2[9], _t1[3], _t2[3]):	#from : http:#people.csail.mit.edu/bkph/articles/Essential.pdf
#ifdef USE_EIGEN
	using namespace Eigen

	E = Map<Matrix<double,3,3,RowMajor> >(_E)
	EEt = E * E.transpose()
	e0e1 = E.col(0).cross(E.col(1)),e1e2 = E.col(1).cross(E.col(2)),e2e0 = E.col(2).cross(E.col(2))
	Vector3d b1,b2

#if 1
	#Method 1
	bbt = 0.5 * EEt.trace() * Matrix3d.Identity() - EEt; #Horn90 (12)
	bbt_diag = bbt.diagonal()
	if bbt_diag(0) > bbt_diag(1) and bbt_diag(0) > bbt_diag(2):		b1 = bbt.row(0) / sqrt(bbt_diag(0))
		b2 = -b1
	} elif bbt_diag(1) > bbt_diag(0) and bbt_diag(1) > bbt_diag(2):		b1 = bbt.row(1) / sqrt(bbt_diag(1))
		b2 = -b1
	} else:
...
#else:
	#Method 2
	if e0e1.norm() > e1e2.norm() and e0e1.norm() > e2e0.norm():		b1 = e0e1.normalized() * sqrt(0.5 * EEt.trace()); #Horn90 (18)
		b2 = -b1
	} elif e1e2.norm() > e0e1.norm() and e1e2.norm() > e2e0.norm():		b1 = e1e2.normalized() * sqrt(0.5 * EEt.trace()); #Horn90 (18)
		b2 = -b1
	} else:
	...
	#B = [b]_x , Horn90 (6) and http:#en.wikipedia.org/wiki/Cross_product#Conversion_to_matrix_multiplication
...
def CheckCoherentRotation(self, R):
	...
	if fabsf(determinant(R))-1.0 > 1e-07:		cerr << "det(R) != +-1.0, is not a rotation matrix" << endl
		return False
...
Mat GetFundamentalMat( vector<KeyPoint>& imgpts1,
	...
	#(although self is not the proper way to do self)
	...
	for (unsigned int i=0; i<status.size(); i++)		if (status[i]) 
...
#ifdef __SFM__DEBUG__
	if(not img_1.empty() and not img_2.empty()) {		
		...
		c = waitKey(0)
		if c=='s':			imwrite("fundamental_mat_matches.png", img_orig_matches)
...
def TakeSVDOfE(self, E, svd_u, svd_vt, svd_w):#if 1
	#Using OpenCV's SVD
	...
	#Using Eigen's SVD
	cout << "Eigen3 SVD..\n"
	e = Eigen.Map<Eigen.Matrix<double,3,3,Eigen.RowMajor> >((double*)E.data).cast<float>()
...
def TestTriangulation(self, pcloud, P, status):	vector<Point3d> pcloud_pt3d = CloudPointsToPoints(pcloud)
	...
	for (int i=0; i<pcloud.size(); i++)		status[i] = (pcloud_pt3d_projected[i].z > 0) ? 1 : 0

	count = countNonZero(status)

	percentage = ((double)count / (double)pcloud.size())
	...
	if(False) #not
		...
		for(unsigned int i=0;i<pcloud.size();i++)			cldm.row(i)(0) = pcloud[i].pt.x
		...
		num_inliers = 0
		nrm = pca.eigenvectors.row(2); nrm = nrm / norm(nrm)
		x0 = pca.mean
		p_to_plane_thresh = sqrt(pca.eigenvalues.at<double>(2))

		for (int i=0; i<pcloud.size(); i++)			w = Vec3d(pcloud[i].pt) - x0
			D = fabs(nrm.dot(w))
...
bool DecomposeEtoRandT(
	...
	singular_values_ratio = fabsf(svd_w.at<double>(0) / svd_w.at<double>(1))
	if(singular_values_ratio>1.0) singular_values_ratio = 1.0/singular_values_ratio; # flip ratio to keep it [0,1]
	if singular_values_ratio < 0.7:		cout << "singular values are too far apart\n"
		return False
...
bool FindCameraMatrices( Mat& K, 
	...
	#Find camera matrices
		...
		F = GetFundamentalMat(imgpts1,imgpts2,imgpts1_good,imgpts2_good,matches
#ifdef __SFM__DEBUG__
								  ,img_1,img_2
#endif
								  )
		if(matches.size() < 100) { # or ((double)imgpts1_good.size() / (double)imgpts1.size()) < 0.25
		...
		if fabsf(determinant(E)) > 1e-07:			cout << "det(E) != 0 : " << determinant(E) << "\n"
			P1 = 0
			return False
		...
		{			
			if (not DecomposeEtoRandT(E,R1,R2,t1,t2)) return False

			if determinant(R1)+1.0 < 1e-09:				#according to http:#en.wikipedia.org/wiki/Essential_matrix#Showing_that_it_is_valid
				cout << "det(R) == -1 ["<<determinant(R1)<<"]: flip E's sign" << endl
				E = -E
				DecomposeEtoRandT(E,R1,R2,t1,t2)

			if not CheckCoherentRotation(R1):				cout << "resulting rotation is not coherent\n"
				P1 = 0
				return False
			...
			if not TestTriangulation(pcloud,P1,tmp_status) or not TestTriangulation(pcloud1,P,tmp_status) or reproj_error1 > 100.0 or reproj_error2 > 100.0:				P1 = Matx34d(R1(0,0),	R1(0,1),	R1(0,2),	t2(0),
				...
				if not TestTriangulation(pcloud,P1,tmp_status) or not TestTriangulation(pcloud1,P,tmp_status) or reproj_error1 > 100.0 or reproj_error2 > 100.0:					if not CheckCoherentRotation(R2):						cout << "resulting rotation is not coherent\n"
						P1 = 0
						return False
					...
					if not TestTriangulation(pcloud,P1,tmp_status) or not TestTriangulation(pcloud1,P,tmp_status) or reproj_error1 > 100.0 or reproj_error2 > 100.0:						P1 = Matx34d(R2(0,0),	R2(0,1),	R2(0,2),	t2(0),
						...
						if not TestTriangulation(pcloud,P1,tmp_status) or not TestTriangulation(pcloud1,P,tmp_status) or reproj_error1 > 100.0 or reproj_error2 > 100.0:							cout << "Shit." << endl; 
							return False

					}				
				}			

			for (unsigned int i=0; i<pcloud.size(); i++)				outCloud.push_back(pcloud[i])
