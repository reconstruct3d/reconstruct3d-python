...
from points_to_ply import points_to_ply
...
class App:
    ...
    def run(self):
        files_count = len(os.listdir('frames'))
        idxrange = range(1,files_count - 2)
        img0 = cv2.imread(os.path.join(thisdir,'frames/image-%03d.jpg' % (idxrange[0],)))
        w, h = getsize(img0)
        ...
        for imgidx in idxrange:
            print('-------------------------------------------------------')
            print(os.path.join(thisdir,'frames/image-%03d.jpg' % (imgidx,))+'\n'+
                  os.path.join(thisdir,'frames/image-%03d.jpg' % (imgidx+1,)))
            img1 = cv2.imread(os.path.join(thisdir,'frames/image-%03d.jpg' % (imgidx,)))
            img2 = cv2.imread(os.path.join(thisdir,'frames/image-%03d.jpg' % (imgidx+1,)))
            ...
            objectPoints = (points4D.T[:,:3]/ points4D.T[:,3:4]).astype(np.float32)
            print(objectPoints)
            ...
            print(disparity.shape)
            cv2.imwrite('output/keypoints/keypoint_matches-%03d.jpg' % (imgidx), imgpair)
            cv2.imwrite('output/rectified/rectified-%03d.jpg' % (imgidx), rectified)
            cv2.imwrite('output/disparity/disparity-%03d.jpg' % (imgidx), ((disparity+50)*.5).astype(np.uint8))
        ...
        # points_to_ply('output', rectified, rectified)
        plt.close()
        fig, axarr = plt.subplots(2, 2)
        a_, b_ = p0[:,0].astype('int'), 480-p0[:,1].astype('int')
        
        # colors = rgb2hex(img1[b_, a_])
        # img1[b_, a_] = (img1[b_, a_].astype(np.float32) - np.max(img1[b_, a_].astype(np.float32)))/-np.ptp(img1[b_, a_].astype(np.float32))
        # img1[b_, a_] = img1[b_, a_].astype(np.float32) / 255.
        print('Hello')
        # import itertools
        # scaled_color = (img1[b_, a_].astype(np.float64)/255.)
        # print(img1[b_, a_])
        # colors = [ rgb2hex(scaled_color[i,:]) for i in range(scaled_color.shape[0]) ]
        # #print(colors[0][0][0])
        # x = np.arange(10)
        # ax = fig.add_subplot(111, projection='3d')
        # ys = [i+x+(i*x)**2 for i in range(10)]
        # # colors = itertools.cycle(["r", "b", "g"])
        # colors = itertools.cycle(scaled_color)
        # for y in ys:
        #     ax.scatter(p0[:,0], 480-p0[:,1], color=next(colors), s=0.1)
        # plt.draw()
        
        axarr[0,0].scatter(p0[:,0], 480-p0[:,1])
        ...
        # ax.scatter(p1[:,0], 480-p1[:,1],)

        plt.ion()
        plt.draw()
        fig.savefig('output/3d-model.jpg')
